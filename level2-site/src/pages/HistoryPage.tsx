...
const API_BASE = 'https://backend-39m6uewb7-jcmeng2002s-projects.vercel.app'
              ...
              <div key={app.id} className="bg-white rounded-xl p-6 border border-border hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <p className="text-gray-600 text-sm">{app.awardType} · {app.department}</p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm text-gray-600 mb-4">
                  <div className="flex items-center gap-2"><User className="w-4 h-4" /> {app.employeeName}</div>
                  <div className="flex items-center gap-2"><Building className="w-4 h-4" /> {app.department}</div>
                </div>

                <div className="flex items-center justify-between">
                  <div className="flex gap-2">
                    {canSubmit && (
                      <button onClick={() => submitApp(app.id)} className="flex items-center gap-1 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-sm">
                        <Send className="w-4 h-4" /> 提交
                      </button>
                    )}
                    {canEdit && (
                      <button onClick={() => navigate(`/apply?edit=${app.id}`)} className="flex items-center gap-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm">
                        <Edit className="w-4 h-4" /> 编辑
                      </button>
                    )}
                    {canDelete && (
                      <button onClick={() => deleteApp(app.id)} className="flex items-center gap-1 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors text-sm">
                        <Trash2 className="w-4 h-4" /> 删除
                      </button>
                    )}
                  </div>
                  <button onClick={() => setExpandedId(isExpanded ? null : app.id)} className="flex items-center gap-1 px-3 py-1 text-sm text-gray-500 hover:text-gray-700">
