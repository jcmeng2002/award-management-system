...
const Level2Layout = ({ children }: { children: React.ReactNode }) => {
  const { user } = useAuth()
    ...
    <div className="min-h-screen bg-gray-50 flex">
      <aside className="w-64 bg-white border-r border-border flex flex-col">
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          <a href="/" className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-green-50 text-green-700 font-medium">
            <span>首页</span>
          </a>
        ...
        <div className="px-4 py-4 border-t border-border">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-green-100 rounded-full flex items-center justify-center">
              <span className="text-green-700 font-medium">{user?.name?.charAt(0) || 'U'}</span>
            </div>
            <div>
              <p className="font-medium text-secondary text-sm">{user?.name}</p>
              <p className="text-xs text-green-600 font-medium">部门负责人</p>
            </div>
          </div>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col">
        <header className="h-16 bg-white border-b border-border flex items-center justify-between px-6">
          <span className="text-sm text-gray-500">部门负责人入口 · 奖项审核系统</span>
        </header>
