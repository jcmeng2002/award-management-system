...
import { FileText, Search, BookOpen, HelpCircle, History, Clock, CheckCircle, Shield, Settings, Users } from 'lucide-react'
...
const HomePage = () => {
  ...
  const features = [
    ...
    { icon: Users, title: '权限配置', desc: '管理用户权限', path: '/admin?tab=permissions', color: 'bg-purple-600' },
    { icon: Shield, title: '知识库管理', desc: '管理系统知识', path: '/admin?tab=knowledge', color: 'bg-red-500' },
  ]
  ...
  return (
    <div className="p-8 animate-fadeIn">
      <div className="mb-8">
        ...
        <p className="text-gray-600">管理员 · {user?.department}</p>
      </div>
      ...
      <div className="mt-8 bg-purple-50 rounded-xl p-6 border border-purple-200">
        <h2 className="text-lg font-semibold text-secondary mb-4">管理面板</h2>
        <div className="space-y-3">
          <div className="flex items-center gap-3 p-3 bg-white rounded-lg">
            <div className="w-2 h-2 bg-purple-500 rounded-full"></div>
            <span className="text-sm text-gray-700">系统运行正常</span>
          </div>
        </div>
      </div>
    </div>
