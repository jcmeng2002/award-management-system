...
import { AwardApplication, AWARD_TYPES } from '../types'
...
const ApplyPage = () => {
  ...
  useEffect(() => {
    if (editId) {
      ...
      if (app) {
        setForm({
          awardType: app.awardType,
          awardName: app.awardName,
          reason: app.reason,
          contact: app.contact,
        })
      }
  ...
  const handleSave = (submit: boolean) => {
    ...
    if (editId) {
      const index = all.findIndex(a => a.id === editId)
      if (index >= 0) {
        all[index] = {
          ...all[index],
          ...form,
          status: submit ? 'pending_confirm' : 'draft',
          updateDate: new Date().toISOString().split('T')[0],
          submitDate: submit ? new Date().toISOString().split('T')[0] : all[index].submitDate,
        }
      }
    } else {
      const newApp: AwardApplication = {
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fadeIn">
      <div className="mb-8 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-600">填写奖项申请信息</p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            >
              ...
              {AWARD_TYPES.filter(t => t.enabled !== false).map(type => (
